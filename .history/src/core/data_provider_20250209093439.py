...
class YahooDataProvider(DataProvider):
    ...
    def _save_to_db(self, data: pd.DataFrame, conn: sqlite3.Connection) -> None:
        ...
        # If the first column is a tuple (i.e. from a MultiIndex), extract the first element.
        if isinstance(first_col, tuple):
            first_col_str = first_col[0]
            df.rename(columns={first_col: first_col_str}, inplace=True)
        else:
            first_col_str = first_col

        # Rename the first column to 'date' if necessary
        if first_col_str.lower() != 'date':
            df.rename(columns={first_col_str: 'date'}, inplace=True)
